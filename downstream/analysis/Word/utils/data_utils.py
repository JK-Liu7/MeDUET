...
class AugDataset(torch.utils.data.Dataset):
    def __init__(self, base_ds: torch.utils.data.Dataset, aug_transform):
    ...
    def __getitem__(self, idx):
        ...
        return self.aug_transform(item)
...
def get_loader(args):
    ...
    train_det_transform = transforms.Compose(
        [
    ...
    train_aug_transform = transforms.Compose(
        [
            ...
            transforms.EnsureTyped(
                keys=["image"], allow_missing_keys=True, dtype=torch.float32
            ),
        ]
    )
    ...
    val_transform = transforms.Compose(
        [
            ...
            transforms.DivisiblePadd(
                keys=["image", "label"], allow_missing_keys=True, k=4
            ),
            transforms.SpatialPadd(
                keys=["image", "label"],
                spatial_size=args.roi_x,
    ...
    if args.test_mode:
        ...
        if args.use_normal_dataset:
            ...
            cached_ds = PersistentDataset(
                data=datalist,
                transform=train_det_transform,
                pickle_protocol=pickle.HIGHEST_PROTOCOL,
                cache_dir=args.cache_dir,
            )
            train_ds = AugDataset(cached_ds, train_aug_transform)
        else:
            train_transform_full = transforms.Compose(
                train_det_transform.transforms + train_aug_transform.transforms
            )
