...
class SiTBlock(nn.Module):
    """
    A SiT block with AdaLN-Zero where (γ, β) are produced
...
class SiTBlock_VAE_simple(nn.Module):
    """
...
class FinalLayer(nn.Module):
    """
    The final layer of SiT. AdaLN-Zero with separate (time/content/style) modulation.
    """
...
class SiT(nn.Module):
    ...
    def forward_with_cfg(self, x, t, content, style, cfg_scale_c, cfg_scale_s):
        ...
        c_emb = self.content_embedder(content, self.training)
        s_emb = self.style_embedder(style, self.training)
        c_emb_null = self.content_embedder.null_embedding(B, t.device, t.dtype)
        s_emb_null = self.style_embedder.null_embedding(B, t.device, t.dtype)
...
class SiT_VAE_simple(nn.Module):
    """
    Diffusion model with a Transformer backbone.
    """
    def __init__(
        ...
        self.blocks = nn.ModuleList([
            SiTBlock_VAE_simple(hidden_size, num_heads, mlp_ratio=mlp_ratio) for _ in range(depth)
        ])
        self.final_layer = FinalLayer(hidden_size, patch_size, self.out_channels)
        self.initialize_weights()
...
def SiT_B_2_VAE_simple(**kwargs):
    return SiT_VAE_simple(depth=12, hidden_size=768, patch_size=2, num_heads=12, **kwargs)

def SiT_XL_2_VAE_simple(**kwargs):
    return SiT_VAE_simple(depth=28, hidden_size=1152, patch_size=2, num_heads=16, **kwargs)
...
SiT_models = {
    ...
    # custom
    'SiT-B/2-VAE-simple':   SiT_B_2_VAE_simple,
    'SiT-XL/2-VAE-simple':   SiT_XL_2_VAE_simple,
}
